...
_wep	= _this select 2;  
_bullet = nearestObject  [getpos _Man, _ammo];
...
_z = 0;
_zTrue = 0;
//_Man sidechat format ["_Man: %1", _Man];
{
	if(_x == _Man)then
	{
...
//**Другие условия
	if(_Ammo != "cal68_FieldPaint")then 
	{
		
		if(_selection == "Legs")then{if(!canstand _man)then{};_man setHit["Legs", 1];};
		if(_selection == "Hands")then{if(handshit _man == 1)then{};_man setHit["Hands", 1]};																													
	};
//**

//**Стан сработал
	if((_StunMuchLocal>2)and!(_Man in Trouble)and!(playerisdead)) then
	{
  ...
  //** В тачке
	...
	//**
		...
		if(player == _man)then {if!(_StunOverDose > 3)then{[] execVM "stun\StunEffectShotgun.sqf";};}; //включения эффекта
		if(player == _man)then {Player setVariable ["GaveUp", true, true]; StunOverDose = StunOverDose + 1; _StunOverDose = StunOverDose;};
		...
		if ((count _weapons > 0)and(_StunOverDose >1)) then 
		{											
	...
	else
	{
			...
			sleep 30;
			if(!(_man in RestrainedPeople)and(_man in Trouble)and!(playerisdead))then{_man switchmove "AinvPknlMstpSlayWrflDnon_healed2";};
	};
		
		}
		else
		{if(!(_man in RestrainedPeople)and(_man in Trouble)and!(playerisdead))then{_man switchmove "AinvPknlMstpSlayWrflDnon_healed2";};}; //обынчое попадание	
		
		
		if((_man != player)and!(_man in RestrainedPeople))then{_man switchmove "AinvPknlMstpSlayWrflDnon_healed2";};
		sleep 5;
