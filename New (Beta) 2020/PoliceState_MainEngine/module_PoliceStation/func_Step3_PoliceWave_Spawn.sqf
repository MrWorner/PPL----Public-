//func_Step3_PoliceWave_Spawn
private["_PositionMarker","_UnitsArray","_this","_NewName","_name","_marker","_GROUP","_POLICECAR","_DRIVER","_PASSENGER","_x","_CHOSEN_UnitsArray","_CHOSEN_POLICECAR","_CHOSEN_DriversSize","_CHOSEN_DefaultSkin_CrewSize","_CHOSEN_RandomTrueFalse","_CHOSEN_DriverSkin","_CHOSEN_CrewSkin","_CHOSEN_BaseCOST","_CHOSEN_AdditonCrewArray","_MyCount","_y","_Police_RiskLevel_BonusScore","_i","Identified"];
...
_CHOSEN_UnitsArray = _UnitsArray call func_Step2_PoliceWave_ChooseArmy; // To avoid problem with balance
if((count _UnitsArray) == 0)exitwith{ hintc "func_Step3_PoliceWave_Spawn: Army not chosen: Not enough resources OR Limit units"};
if((count _UnitsArray) > 1)then
{
	_CHOSEN_UnitsArray = _UnitsArray select (round (random ( (count _UnitsArray) - 1)));	
}
else
{
	_CHOSEN_UnitsArray = _UnitsArray select 0;
};
...
if(_MyCount > 0)then
{
	...
	for [{_x=1},{_x<=_MyCount},{_x=_x+1}] do
	{
			if( (_CHOSEN_AdditonCrewArray select (_y+1)) >= (random 100))then
			{
				...
				//_y Spec Crew, 
				//_y+1 Chance to be spawned
				//_y+2 Identity, 
				//_y+3 WEAPON CLASS, 
				//_y+4 bonus score,
				
				_Identified = _Identified + [_PASSENGER];
				_PASSENGER = _GROUP createUnit [_CHOSEN_CrewSkin, _POLICECAR, [], 1, "NONE"];
				_PASSENGER setVariable ["Police_RiskLevel_BonusScore",(_CHOSEN_AdditonCrewArray select (_y+4)),true];
				_PASSENGER setIdentity (_CHOSEN_AdditonCrewArray select (_y+2) );
				_PASSENGER setVariable ["MyWepRole",(_CHOSEN_AdditonCrewArray select (_y+3) )];
			}
			else
			{
				_CHOSEN_CrewSkin =  _CHOSEN_UnitsArray select 5;
