//fnc_SurrenderOrFight_BW
private["_x","_this","_susp","_morale","_NeedToSay","_result","_friends","_friendsRealTime","_pressure","_COP"];
...
{
	if(_x getVariable "BW_ChaseInProgress" > 0)then
	{
		if(_x distance _susp < 30)
		then
		{
			if(!(_x getVariable "GivingUp")and!(_x getVariable "InFighting"))
			then
			{
				...
				_morale = _morale + _friendsPower;
				 ...
				 if((_x getVariable "BW_Dangerous"))then{_morale = _morale + 15};
				 
				 //ADDINITON COPS
				 _pressure = 30;
				 if(_NeedToSay)then{_pressure = _pressure + 20;}; 			 
				 _pressure = _pressure + (2*(_x getVariable "BW_ChaseInProgress"));
				...
				{//say NO!
					...
					if(_NeedToSay)then
					{
						...
						{
							if( (count _friendsRealTime < 3)and( (_x getVariable "BW_ChaseInProgress") > 2))then
							{
								_addMore = (-50) - (10*(_x getVariable "BW_ChaseInProgress"));
							}
							else
							{
								_addMore = 0 - (5*(_x getVariable "BW_ChaseInProgress"));
							};	
						};
